"""Runs the frozen `codex-canonical-v1` corpus through the real
`OpenAIGateway`, one `run_corpus` call per repository (each against its
own ingested graph), and produces a comprehensive report: per-case,
per-category, per-repository, token usage, and aggregate deterministic
metrics.

Loads the corpus verbatim from the checked-in frozen fixture -- never
rebuilds it. Saves a full JSON artifact (every `CaseRunResult`,
including raw output) to `benchmark_runs/` so the run is inspectable
after the fact, not just summarized to stdout.
"""
...
FROZEN_FIXTURE = REPO_ROOT / "tests" / "fixtures" / "benchmark" / "codex_canonical_corpus_v1.json"
...
def main() -> int:
    ...
    click_evidence = InMemoryEvidenceStore()
    click_result = IngestionPipeline(click_registry, click_evidence).run(click_repo)
    ...
    flask_evidence = InMemoryEvidenceStore()
    flask_result = IngestionPipeline(flask_registry, flask_evidence).run(flask_repo)
    ...
    out_path = OUT_DIR / "canonical_v1_openai_run.json"
    ...
    for repo_id, record in all_records.items():
        for qid, case_result in record.results.items():
            ...
            print(f"\n[{repo_id}] {category.value} — {case.query_text!r}")
            print(f"  generation_status: {case_result.generation_status}")
            print(f"  error: {case_result.error}")
            print(f"  finish_reason: {case_result.finish_reason}")
