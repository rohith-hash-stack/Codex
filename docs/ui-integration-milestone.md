...
> **RESOLVED** in the follow-up Canonical Identity Resolution Fix milestone — see `docs/canonical-identity-resolution-fix.md` for the root cause, the minimal fix, and confirmation that the `AskPanel.runExpand` fallback described below was removed. This section is left as written at the time of the UI Integration Milestone (a snapshot of its own point in time, not corrected in place), per this project's own "documentation only, never alter historical validation claims" discipline.
