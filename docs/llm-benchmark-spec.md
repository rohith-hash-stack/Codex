...
## 7. The real run: environment network policy, not a code defect

Before writing any gateway code, `curl -sS https://api.openai.com/v1/models` was attempted directly and failed with `CONNECT tunnel failed, response 403`; the session's own egress-proxy status endpoint confirms this is a hard **organization policy denial** (`api.openai.com` is not in the proxy's allowlist, which permits only a fixed set of hosts — Anthropic's own API, npm/PyPI/crates/Go module registries — and nothing else). This is an environment-level restriction, not a retryable transient failure, and this project's own standing instructions are explicit that TLS verification must never be disabled and `HTTPS_PROXY` must never be unset to route around it — so no workaround was attempted.

`scripts/run_openai_benchmark.py` was run anyway, against the real, frozen `codex-self-dev-v0` corpus and the real, live-ingested repository, with a real `OpenAIGateway()`. The result: every reproducibility dimension recorded correctly, real D8/D9 retrieval ran for all 4 cases, real `PRECISION_AT_10`/`RECALL_AT_10`/`MRR` were computed against the frozen ground truth — and all 4 cases' `CaseRunResult.error` read `"transport failure contacting OpenAI: <urlopen error Tunnel connection failed: 403 Forbidden>"`, with `generation_status=None`, `served_model=None`, no OpenAI response ever obtained. This proves the entire pipeline is wired correctly end-to-end; the one link this sandboxed environment cannot complete is the outbound call itself.

**Nothing about this is a fallback**: no other provider was tried, no result was fabricated, and the failure is loud and fully captured (per §6). Rerunning `scripts/run_openai_benchmark.py` from an environment whose egress policy allows `api.openai.com` (or with the proxy allowlist updated to include it) requires no code change.
