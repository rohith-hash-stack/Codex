...
**Amendment (D1-D10 Integration Hardening Pass, 2026-08-31):** the two findings recorded in §12 below (symbol-level E2E gap, `provider_authority` gap) were closed per a dedicated hardening directive — full detail in `docs/architecture-conformance-audit.md` §W. §12, §15, and §16 below are updated in place to reflect the resolution (marked inline); §1-11, §13-14 are unchanged and remain accurate as originally audited. Summary: Finding 1 closed via a new, additive, backward-compatible test fixture (`tests/symbol_level_fixtures.py`, `tests/test_symbol_level_e2e.py`) proving the full D1-D10 chain for a symbol-level query — zero production code changed, since production retrieval code was already correct. Finding 2 closed via a small, additive, backward-compatible wiring (`CapabilityRegistry.provider_authority_map()` + `run_verification_loop(..., registry=...)`) after determining D2 already holds sufficient authoritative data (`ProviderScoreProfile.evidence_quality`) — no new definition was invented. 663/663 tests passing, 99% coverage, clean ruff/mypy after the change. D11 still not started.
...
**Fresh validation, as originally audited (pre-hardening-pass):**
...
**Post-hardening-pass (2026-08-31, see `docs/architecture-conformance-audit.md` §W): 663 passed, 99% coverage (3006 statements, 19 missed — the 12 new tests' own modules stay at 100%), ruff/mypy still clean.**
...
### Two genuine, previously-undocumented findings — both now CLOSED (§W)

**Finding 1 — `DeterministicFakeAdapter` is FILE-only (test-infrastructure gap, not a production defect). CLOSED.** `tests/fake_ingestion_provider.py`'s `DeterministicFakeAdapter.normalize()` hardcoded `base_type=BaseEntityType.FILE` at all 4 entity-construction call sites — confirmed by grep before this pass began. Since `tests/planner_fixtures.py`'s `build_graph()` is built exclusively on this fixture and is used by every D9 test and every D10 integration test in `test_d1_d10_integration.py`, **no existing D9/D10 test suite had ever exercised symbol/function/method-level retrieval** — only file-level — even though HLRD/TAD's own worked examples (HLRD §36: "Who calls `PaymentService`?"; this directive's own primary query, "Which tests call `authenticate`?") are function/class-level, not file-level.

Root-caused via the primary trace (§4): a manually-constructed FUNCTION-level graph (bypassing the fixture) proved `GraphReader.find_entities`, `codex.planner.retrieval.resolve_targets`, and `codex.planner.retrieval.bounded_traversal` are **already fully correct** for symbol-level entities — the gap was exclusively in the test double's construction logic, never in production code. At the time this report was first written, per the "only a clearly proven regression/defect authorizes a code change" rule, no fix was made — it was reported as a recommendation for a separately-scoped follow-up task.

**Closed 2026-08-31** by the dedicated hardening pass (`docs/architecture-conformance-audit.md` §W.1): `DeterministicFakeAdapter` gained two optional, backward-compatible constructor parameters (`base_type`, `object_base_type`, both defaulting to the pre-existing `FILE`-only behavior — every prior test unaffected); a new `tests/symbol_level_fixtures.py`/`tests/test_symbol_level_e2e.py` proves the full real chain — `codex.query_understanding` (D8) → `codex.planner` (D9) → `codex.llm`/`codex.verification` (D10) — for a symbol-level query (`"Which tests call authenticate?"`, driven by real Tier-0 text detection, not a hand-built contract) plus a second `CLASS`→`CONTAINS`→`METHOD` shape. **Zero lines of `codex.ontology`/`codex.graph`/`codex.planner` were changed** — production retrieval code needed no fix, exactly as this report's original finding concluded.

**Finding 2 — `provider_authority` (TAD §48, weight 0.10) is not wired to real provider-scoring data in any production call path. CLOSED.** `run_verification_loop`/`verify_claims`/`verify_claim`/`compute_factors` all accept `provider_authority: Mapping[str, float] | None = None` as an optional caller-supplied override; when omitted (as every real invocation observed this pass does — confirmed by grep, no call site in `src/codex/` outside tests ever supplies this argument), `authority.get(provider, 1.0)` defaults every provider to `1.0`. `codex.registry.scoring.ProviderScoreProfile`/`evidence_quality` (D2, already implemented and populated at `CapabilityRegistry.register()` time) is never read by anything in `codex.verification`. This means the `provider_authority` confidence factor currently contributes a constant, uninformative `0.10 * 1.0` to every `V` computed by real code today, including the live trace in §4. Not a spec violation (TAD §48 doesn't mandate the sourcing mechanism) and not incorrect (uniform 1.0 is a safe default, not a wrong value) — but it meant D2's already-built provider-authority signal provided zero benefit to D10's verification confidence in practice.

**Closed 2026-08-31** by the dedicated hardening pass (`docs/architecture-conformance-audit.md` §W.2), after explicitly determining D2 already holds sufficient authoritative data — no new definition was invented: `CapabilityRegistry.provider_authority_map()` (new, additive accessor) exposes `{provider_name: profile.evidence_quality}`, checked to confirm it is not a duplicate of TAD §48's *own*, differently-scoped `evidence_quality` factor (which averages retrieved `Evidence.confidence` per query, never registry data — the two factors never read the same numbers, despite the shared field name). `run_verification_loop()` gained an optional `registry: CapabilityRegistry | None = None` parameter; an explicit `provider_authority` mapping still always wins, and omitting both arguments (every pre-existing caller) is byte-identical to the pre-hardening default. 9 new regression tests prove the wiring actually changes computed confidence, override precedence, and safe fallback for an unregistered provider.
...
| Verification Engine — `provider_authority` factor specifically | **Layer 1 — Implemented + integrated** (was Layer 2 as originally audited; closed 2026-08-31, `docs/architecture-conformance-audit.md` §W.2) — `CapabilityRegistry.provider_authority_map()` wired into `run_verification_loop(..., registry=...)`, proven to change computed `V` with real registry data, not just accept the parameter (§12 Finding 2) |
...
| Symbol/function-level retrieval through the D9/D10 *test suite* | **Layer 1 — Implemented + integrated** (was Layer 2 as originally audited; closed 2026-08-31, `docs/architecture-conformance-audit.md` §W.1) — `tests/symbol_level_fixtures.py`/`tests/test_symbol_level_e2e.py` now exercise the full D8→D9→D10 chain for a symbol-level query end to end, on top of the still-unmodified, already-correct production retrieval code (§4/§12 Finding 1) |
...
**GO WITH CONDITIONS.** (Re-affirmed 2026-08-31 after the D1-D10 Integration Hardening Pass, `docs/architecture-conformance-audit.md` §W — two of the seven conditions below are now closed.)

D1-D10 are architecturally coherent with HLRD v1.0/TAD v1.0. No blocking contradiction was found — the only two candidate "contradictions" investigated in depth (`EvidencePackage.query_contract` vs. `query_identity`, and the `MAX_ATTEMPTS=2` vs. "maximum re-synthesis: 1" naming) both resolved to intentional, non-blocking implementation choices under scrutiny, not real conflicts. The codebase is clean (663/663 tests as of the hardening pass, 99% coverage, zero lint/type errors, zero drift markers) and a live, real end-to-end trace against a realistic function-level repository graph produced the correct answer through every stage — now also proven as a committed regression test (§12 Finding 1).

**Conditions carried forward from the original audit, updated in place:**

1. TAD §33's completeness-percentage denominator remains genuinely undefined in TAD itself — needs a human/benchmark decision before Phase 5 calibration, not before D11. **Still open** — out of the hardening pass's scope (a specification gap, not an integration gap).
2. ~~`provider_authority` (D10's confidence formula) is not yet wired to D2's already-built `ProviderScoreProfile` data in any real call path.~~ **CLOSED 2026-08-31** — `CapabilityRegistry.provider_authority_map()` + `run_verification_loop(..., registry=...)`, per §12 Finding 2 / `docs/architecture-conformance-audit.md` §W.2. D2 already held sufficient authoritative data; nothing was invented.
3. ~~`DeterministicFakeAdapter` (the shared D9/D10 test fixture) is FILE-only, so the D9/D10 test *suite* has never exercised symbol/function-level retrieval end to end.~~ **CLOSED 2026-08-31** — `tests/symbol_level_fixtures.py`/`tests/test_symbol_level_e2e.py`, per §12 Finding 1 / `docs/architecture-conformance-audit.md` §W.1. Zero production code changed — retrieval code was already correct, only the test double needed extending.
4. D7 (Sourcegraph/RepoGraph) remains deferred behind its filed STOP (ADR-006 open) — real multi-provider independence corroboration stays synthetic-only until it's resolved. **Still open** — out of scope for a test/wiring hardening pass; blocked on external licensing/determinism evidence.
5. `CONCURRENT_UPDATE_DETECTED` (TAD §55/§64) has no implementation anywhere. **Still open** — not addressed by this hardening pass (it targeted only the two findings named in the hardening directive).
6. Storage/cache/API/auth/deployment ADRs (§15, Layer 5) remain open, as TAD §84 always expected at this stage. **Still open**, unaffected by this pass.
7. Benchmark/performance validation (TAD §80 Phase 5) has not started, correctly so. **Still open**, unaffected by this pass.

**Recommended next phase** (per TAD's own ordering, not started under this or the hardening directive): TAD §80 lists Phase 5 (Validation: benchmark repositories, ground truth, metrics, calibration, failure testing) as the phase after D10's "Phase 4 — Reasoning" is complete. Item 5 (`CONCURRENT_UPDATE_DETECTED`) remains a small, contained, non-architectural task worth closing alongside any future hardening pass; item 1 needs a human decision before calibration can proceed meaningfully; item 4 (D7) stays blocked on external licensing/determinism evidence this environment cannot resolve on its own.

D11 was not started, in either this audit or the hardening pass that followed it.
